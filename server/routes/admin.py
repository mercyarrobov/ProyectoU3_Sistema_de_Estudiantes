...
def createUser():
    """ 
    Funcion que envia los datos de usuarios
    Crea al usuario con los datos de nombre, apellido, username, password, numero de cedula, rol y la imagen del usuario.
    """
    #reciving datap
    ...
    # -------- END DATA ------------- # 
    #Verifica los datos del usuarios
    if username and nombre and password and apellido and dni and rol :
        #Los comprueba con el nombre del rol
        ...
        #Si esta correcto todo lo agrega a la base de datos.
        ...
        #Si no encuentra muestra un error 200 en el estatus 
...
def getUsers():
    """ Función que obtiene los datos y los envia mediante una conversión para json"""
    datos = mydb.users.find()
    users = [ dict(row) for row in datos ]
    #Retorna la conviersión de un objeto Python en una cadena json.
...
def deleteUser():
    ...
    #Si el username existe lo elimina en la base de datos
    if datos:
        ...
        return  response
    #Y muestra un mensaje cuando el username no fue encontrado en el json para eliminar.
    result =  jsonify({
